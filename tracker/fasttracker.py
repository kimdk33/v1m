...
class STrack(BaseTrack):
    ...
    def __init__(self, tlwh, score, cls=None):
    ...
    def to_dict(self):
        ...
        return {
            ...
            "cls": int(self.cls) if self.cls is not None else None,
    ...
    def update(self, output_results, img_info, img_size):
        ...
        if len(dets) > 0:
            ...
            detections = [STrack(STrack.tlbr_to_tlwh(tlbr), s, c) for
              (tlbr, s, c) in zip(dets, scores_keep, classes[remain_inds])]
        else:
            detections = []
        ...
        if len(dets_second) > 0:
            ...
            detections_second = [STrack(STrack.tlbr_to_tlwh(tlbr), s, c) for
                     (tlbr, s, c) in zip(dets_second, scores_second, classes[inds_second])]
        else:
            detections_second = []
